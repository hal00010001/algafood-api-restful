 ...
 * 
 * @Override public List<Cozinha> listar() { return
 * manager.createQuery("from Cozinha", Cozinha.class).getResultList(); }
 * 
 * @Override public Cozinha buscar(Long id) { return manager.find(Cozinha.class,
 * id); }
 * 
 * @Override
 * 
 * @Transactional public Cozinha salvar(Cozinha cozinha) { return
 * manager.merge(cozinha); }
 * 
 * @Override
 * 
 * @Transactional public void remover(Long id) { Cozinha cozinha = buscar(id);
 * if(cozinha == null) { throw new EmptyResultDataAccessException(1); }
 * manager.remove(cozinha); }
 * 
 * @Override public List<Cozinha> consultarPorNome(String nome) { return
